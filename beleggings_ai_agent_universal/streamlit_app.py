...
# Robust import bootstrap: add the folder containing 'src' to sys.path,
# whether this app lives at repo root or in a nested subfolder.
APP_DIR = Path(__file__).resolve().parent
if (APP_DIR / "src").exists() and str(APP_DIR) not in sys.path:
    sys.path.insert(0, str(APP_DIR))
# Also add repo root (parent) in case 'src' sits there.
